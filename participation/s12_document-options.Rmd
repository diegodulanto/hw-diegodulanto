...
```{r, echo=knitr::is_html_output(), fig.width = 10, fig.height = 10, dev="svglite"}
mpgplot <- ggplot2::mpg %>% 
  ggplot() +
  aes(x = displ,
      y = hwy,
...
The overall relationship between highway and displ seems to be negative. The chevrolet manufacturer seems to have an outlier for the year 2008.
...
knitr::kable(mpgtable)

```

The mean and standard deviation for displ increased from 1999 to 2008. 
The mean highway had a slight increase and the standard deviation a slight decrease from 1999-2008.
