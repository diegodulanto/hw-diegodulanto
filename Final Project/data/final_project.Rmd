...
colombia_averages <- immigrant_data %>%
  ...
  summarise(
    col_age = mean(Age, na.rm = TRUE),
    col_MoA = Mode(Month_of_Admission, na.rm = TRUE))

peru_averages <- immigrant_data %>%
  filter(Country_of_Birth == 224) %>%
  group_by(Year_of_Admission) %>%
  summarise(
    peru_age = mean(Age, na.rm = TRUE),
    peru_MoA = Mode(Month_of_Admission, na.rm = TRUE))

venezuela_averages <- immigrant_data %>%
  filter(Country_of_Birth == 227) %>%
  group_by(Year_of_Admission) %>%
  summarise(
    venz_age = mean(Age, na.rm = TRUE),
    venz_MoA = Mode(Month_of_Admission, na.rm = TRUE))
...
ggplot(venezuela_averages) +
  aes(x = venz_age, 
      y = venz_MoA) +
  geom_point() +
  scale_color_brewer(palette = "Dark2") +
  labs(title = "Ecuadorian Immigration Patterns") +
  theme_classic()+
  theme(legend.position = "right")
```
