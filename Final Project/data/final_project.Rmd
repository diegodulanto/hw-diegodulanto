...
sa_contries <- immigrant_data %>%
  mutate(
    recode(Country_of_Birth, 218, 224, 230 == "South American Countries"))

peru_averages <- immigrant_data %>%
  filter(Country_of_Birth == 224) %>%
  summarise(
    mean_age = mean(Age, na.rm = TRUE),
    mode_MoA = Mode(Month_of_Admission, na.rm = TRUE),
    mode_YoA = Mode(Year_of_Admission, na.rm = TRUE))
